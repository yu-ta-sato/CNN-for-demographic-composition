...
def crop_images(
    ...
    viirs              : A boolean indicating the usage of VIIRS.
    ...
    xds_viirs = xds_viirs.where(xds_viirs < 1e10, 0)
    # print(xds_viirs)
...
class RemoteSensingCNN(nn.Module):
    ...
    def __init__(self, out_dim, pretrained):
        ...
        self.conv_all = nn.Conv2d(
            in_channels=64 + 64 + 64, out_channels=64, kernel_size=1, bias=False
        )
    ...
    def forward(self, x):
        ...
        x = torch.cat((band2_4, band5_7, viirs), dim=1)
        x = self.conv_all(x)
        x = self.resnet.bn1(x)
